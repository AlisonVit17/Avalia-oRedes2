...
#ip = input('digite )
ip = '192.168.1.14' #ip do servidor (ip da maquina)
...
ler_json_bloqueado(ip_address[0], '172.18.0.2', 0)
print(ip_address[0], '\n')
...
while(True):
    try:
        ...
        if msg == 'disconnect':
            ...
            ler_json_bloqueado(ip_address[0], '172.18.0.2', 1)
            break    
        ...
        if(lista_hosts[int(host)] != ip_address[0]):
            ...
            caminhoAjeitado = list(caminho_a_seguir)
            del caminhoAjeitado[1]
            caminhoAjeitado = caminhoAjeitado[0]
            del caminhoAjeitado[0]

            print(caminhoAjeitado, '\n\n')
            caminho_a_seguir = caminhoAjeitado
            #Ajeitando a mensagem:
